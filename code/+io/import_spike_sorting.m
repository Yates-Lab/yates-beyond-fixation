function varargout = import_spike_sorting(DataFolder, spikesTag, overwrite)
...
if ~isempty(processedDir)
    ...
    spFile = fullfile(DataFolder, processedDir.name, sprintf('spikes-%s.mat', spikesTag));
%     spFile = dir(fullfile(DataFolder, processedDir.name, sprintf('spikes-%s*.mat', spikesTag)));
    
    if isempty(spFile) || overwrite
        fprintf('Importing spikes file [%s]\n', spFile)
        opsfile = fullfile(DataFolder, processedDir.name, 'ops.mat');
        ops = load(opsfile);
        einfo = load(fullfile(DataFolder, processedDir.name, 'ephys_info.mat'));
        ops = io.convertOpsToNewDirectory(ops, DataFolder); %fullfile(DataFolder, processedDir.name));

        sp = io.getSpikesFromKilo(ops, einfo);
        save(spFile, '-struct', 'sp')
    end


    fprintf('Loading spikes file [%s]\n', spFile)
    sp = load(spFile);
