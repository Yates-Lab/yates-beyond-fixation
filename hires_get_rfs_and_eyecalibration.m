...
%%
% this line is show and we should probably just replace the saved files
% with this output
Exp = fix_mitchelllab_exports(Exp);

%% get coarse receptive fields using the dot stimulus

% initialize the gaze-contingent region of interest near the fovea
ROI = [-1 -1 1 1]*3;
binSize = .25;
Frate = 120;
eyeposexclusion = 20;
...
% this bins up the dots in a gaze-contingent window as described in Yates
% et al., 2023 for figure 2
...
% forward correlation averaging across all cells to find the ROI
...
%% Do high-res reconstruction for the Gabor stimulus
...
% x = Exp.vpx.smo(300:600,2);
x = Exp.vpx.smo(1:100e3,2);
x = x - mean(x);

figure(1); clf
plot(x); hold on; plot(diff(x))
...
%% TODO: just fit a linear interpolant function
